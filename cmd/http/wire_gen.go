...
func Initialize() (*app.RestServer, func(), error) {
	...
	v2Router := v2.New(v4...)
	boltShipmentRepository := repository.NewBoltShipmentRepository(db)
	shipmentQuery := usecase.NewShipmentQuery(boltShipmentRepository)
	createShipmentCommand := usecase.NewCreateShipmentCommand(boltShipmentRepository)
	createShipmentItemCommand := usecase.NewCreateShipmentItemCommand(boltShipmentRepository)
	changeShipmentStateCommand := usecase.NewChangeShipmentStateCommand(boltShipmentRepository)
